"""JSONL sequence adapter for 9-node and 5-node skating IMU samples.

Supports two sensor layouts:
  - 9-node (default): head, elbows, wrists, knees, feet
  - 5-node: waist, knees, ankles

Use ``BASELINE_NODE_ORDER`` for 9-node or ``BASELINE_NODE_ORDER_5NODE`` for
5-node when constructing a ``SequenceConfig``.  The ``resolve_node_mapping``
helper automatically selects the correct JSONL→model node mapping.
"""
...
# ── 5-node configuration (waist + knees + ankles) ──────────────────────
# 保留上述 9 节点链路不变，新增 5 节点链路。5 节点覆盖腰部与下肢：
#   waist, left_knee, right_knee, left_ankle, right_ankle
# 实测中脚踝节点在原始 JSONL 里可能名为 left_foot / right_foot 或
# left_ankle / right_ankle，因此映射表同时收纳两种写法。

JSONL_EXPECTED_NODE_ORDER_5NODE = (
    "waist",
    "left_knee",
    "right_knee",
    "left_ankle",
...
JSONL_TO_MODEL_NODE_MAPPING_5NODE = {
    "waist": "waist",
    "left_knee": "l_knee",
    "right_knee": "r_knee",
    "left_ankle": "l_ankle",
...
def resolve_node_mapping(node_order: Tuple[str, ...]) -> Dict[str, str]:
    """根据 node_order 自动选择 9 节点或 5 节点的 JSONL→model 映射。

    判定逻辑：若 node_order 包含 ``waist`` 且不包含 ``head``，使用 5 节点
    映射；否则回退到 9 节点映射。这保证 ``convert_record_to_sequence`` 能
    正确解析两种传感器布局的数据。
    """
    if "waist" in node_order and "head" not in node_order:
        return dict(JSONL_TO_MODEL_NODE_MAPPING_5NODE)
    return dict(JSONL_TO_MODEL_NODE_MAPPING)
...
def convert_record_to_sequence(
    ...
    node_mapping = resolve_node_mapping(config.node_order)
    ...
    for frame_index, frame in enumerate(sorted_frames):
        ...
        for raw_node_name, raw_values in node_payload.items():
            mapped_node = node_mapping.get(str(raw_node_name))
