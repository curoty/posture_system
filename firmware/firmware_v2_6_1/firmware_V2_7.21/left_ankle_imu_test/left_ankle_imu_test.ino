...
constexpr char MQTT_COMMAND_TOPIC[] = "sensor/imu/commands/left_ankle";
// Factory STA MAC of the left ankle ESP32-S3, compact lowercase form.
constexpr char DEVICE_ID[] = "d40592_49c394";
constexpr char NODE_NAME[] = "left_ankle";
constexpr char FIRMWARE_BUILD[] = "left_ankle-static-v7-cal-guard-20260720";
...
bool connectMqtt() {
  ...
  String clientId = "left_ankle_test_" + WiFi.macAddress();
  clientId.replace(":", "");
...
bool publishMqttBatch(const MqttBatch &batch) {
  ...
  int written = snprintf(
      ...
      "{\"source\":\"left_ankle_imu_test\",\"device_id\":\"%s\","
      "\"sample_rate_hz\":50,"
      "\"filter_status\":\"%s\","
...
void mqttPublishTask(void *) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(mqttQueue, &batch, pdMS_TO_TICKS(100)) == pdTRUE) {
      ...
      // 网络瞬断时保留当前分块并短暂重试，避免左脚节点“在线但数据断档”。
...
void loop() {
  ...
  if (static_cast<int32_t>(nowUs - nextSampleUs) >= 0) {
    ...
    if (readImu(sample)) {
      ...
      if (SERIAL_RAW_DEBUG) {
        Serial.printf(
            "[LEFT_ANKLE_RAW] uptime_ms=%lu unix_ts_ms=%llu time_synced=%s seq=%lu "
