...
// ============================================================
// Left Knee ICM20602 standalone test
// - 50 Hz acquisition
...
constexpr char MQTT_COMMAND_TOPIC[] = "sensor/imu/commands/left_knee";
// Factory STA MAC of the left knee ESP32-S3, compact lowercase form.
constexpr char DEVICE_ID[] = "d40592_48f594";
constexpr char NODE_NAME[] = "left_knee";
constexpr char FIRMWARE_BUILD[] = "left_knee-static-v7-cal-guard-20260720";
...
bool connectMqtt() {
  ...
  String clientId = "left_knee_test_" + WiFi.macAddress();
  clientId.replace(":", "");
...
bool publishMqttBatch(const MqttBatch &batch) {
  ...
  int written = snprintf(
      ...
      "{\"source\":\"left_knee_imu_test\",\"device_id\":\"%s\","
      "\"sample_rate_hz\":50,"
      "\"filter_status\":\"%s\","
...
void loop() {
  ...
  if (static_cast<int32_t>(nowUs - nextSampleUs) >= 0) {
    ...
    if (readImu(sample)) {
      ...
      if (SERIAL_RAW_DEBUG) {
        Serial.printf(
            "[LEFT_KNEE_RAW] uptime_ms=%lu unix_ts_ms=%llu time_synced=%s seq=%lu "
