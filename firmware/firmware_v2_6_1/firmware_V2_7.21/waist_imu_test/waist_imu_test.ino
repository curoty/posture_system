...
// ============================================================
// Waist ICM20602 standalone test
// - 50 Hz acquisition
// - ICM20602 hardware DLPF only
// - user-triggered gyroscope bias calibration
// - raw six-axis + die temperature + timestamp over Serial
// - MQTT batch upload; no ESP-NOW or software attitude/filter algorithm
// ============================================================

// NTP is optional. Sampling still works when WiFi is unavailable.
constexpr char WIFI_SSID[] = "星野与我";
constexpr char WIFI_PASSWORD[] = "hoshino521";
// Give the ESP32 WiFi state machine enough time to finish an association.
// Re-starting WiFi every few seconds interrupts an in-progress connection and
// produces "sta is connecting, return error".
constexpr uint32_t WIFI_CONNECT_TIMEOUT_MS = 30000;
constexpr uint32_t WIFI_RECONNECT_INTERVAL_MS = 5000;
constexpr uint32_t WIFI_RECONNECT_ATTEMPT_TIMEOUT_MS = 10000;
...
constexpr char MQTT_COMMAND_TOPIC[] = "sensor/imu/commands/waist";
// Factory STA MAC of the waist ESP32-S3, compact lowercase form.
constexpr char DEVICE_ID[] = "a0f262f05508";
constexpr char FIRMWARE_BUILD[] = "waist-static-v8-net-recovery-20260721";
// Printing a full JSON-like line at 50 Hz can fill the USB CDC/JTAG TX
// buffer when no Serial Monitor is open and block the sampling loop. Keep
// this disabled for unattended MQTT collection; enable only for short tests.
constexpr bool SERIAL_RAW_DEBUG = false;
// USB CDC Serial.printf can block for seconds when no serial monitor is
// actively draining the endpoint. The old five-second status line caused the
// exact 2.5-second MQTT gap visible in the FastAPI logs.
constexpr bool SERIAL_STATUS_DEBUG = false;
// 25 frames = 0.5 s per MQTT packet at 50 Hz. Publishing five-frame
// packets required 10 public-network MQTT writes per second and overflowed
// the queue on normal WAN latency.
...
constexpr uint8_t SAMPLE_RATE_DIVIDER = 19; // 1 kHz / (19 + 1) = 50 Hz
constexpr uint8_t GYRO_DLPF_CONFIG = 0x04;  // about 20 Hz
constexpr uint8_t ACCEL_DLPF_CONFIG = 0x04; // about 21.2 Hz
...
bool calibrateGyroscope() {
  ...
  if (valid < GYRO_CALIBRATION_SAMPLES * 9 / 10) {
    Serial.printf("[CAL] rejected: only %u/%u valid samples\n", valid,
  ...
  const float stdX =
      sqrtf(fmaxf(0.0f, static_cast<float>(sumSqX / valid) - meanX * meanX));
  const float stdY =
      sqrtf(fmaxf(0.0f, static_cast<float>(sumSqY / valid) - meanY * meanY));
  const float stdZ =
      sqrtf(fmaxf(0.0f, static_cast<float>(sumSqZ / valid) - meanZ * meanZ));
...
void maintainWiFi() {
  ...
  if (wifiConnectInProgress &&
      millis() - wifiConnectStartedMs < WIFI_RECONNECT_ATTEMPT_TIMEOUT_MS)
    return;
  uint32_t now = millis();
  if (now - lastWifiReconnectAttemptMs < WIFI_RECONNECT_INTERVAL_MS) return;
  lastWifiReconnectAttemptMs = now;
  wifiReconnectAttempts++;
  Serial.printf("[WIFI] reconnecting #%u to SSID=%s status=%d\n",
                wifiReconnectAttempts, WIFI_SSID, static_cast<int>(WiFi.status()));
...
bool connectMqtt() {
  ...
  String clientId = "waist-test-" + WiFi.macAddress();
  clientId.replace(":", "");
...
bool publishMqttBatch(const MqttBatch &batch) {
  ...
  int written = snprintf(
      ...
      "{\"source\":\"waist_imu_test\",\"device_id\":\"%s\","
      "\"sample_rate_hz\":50,"
      "\"filter_status\":\"%s\","
  ...
  for (uint8_t i = 0; i < batch.count; ++i) {
    ...
    // Compact wire format:
    // [uptime_ms, unix_ts_ms, time_synced, seq, temperature_c,
    //  ax, ay, az, gx, gy, gz]
    // The server expands this back to the normal frame object. Avoiding
    // repeated JSON keys/device IDs cuts a 25-frame MQTT packet by more than
    // half, preventing the WAN publisher from falling behind acquisition.
    written = snprintf(
...
void setup() {
  ...
  // About 10 seconds of buffering at 2 packets/second. This absorbs short
  // hotspot stalls without affecting the 50 Hz sampling task.
...
void loop() {
  ...
  if (gyroCalibrationRequested) {
    ...
    if (!accepted) {
      ...
      gyroCalibrationValid = false;
      if (previousValid) {
        Serial.println("[CAL] rejected; offset reset, use bench offset");
      }
    }
  ...
  if (static_cast<int32_t>(nowUs - nextSampleUs) >= 0) {
    ...
    if (readImu(sample)) {
      ...
      if (SERIAL_RAW_DEBUG) {
        Serial.printf(
            "[WAIST_RAW] uptime_ms=%lu unix_ts_ms=%llu time_synced=%s seq=%lu "
